...
- **Development Mode:** Raw JSON with live compilation and hot-reload capabilities
...
private:
    PackageManifest parsePackageManifest(const std::string& packagePath) {
        auto jsonDoc = JsonParser::parseFile(packagePath);
        
        PackageManifest manifest;
        manifest.id = jsonDoc.getAttribute("id");
        manifest.name = jsonDoc.getAttribute("name");
        manifest.version = jsonDoc.getAttribute("version");
        manifest.sourcePath = packagePath;
        
        // Parse dependencies
        auto depNodes = jsonDoc.selectNodes("//dependencies/dependency");
        ...
        parseAssetManifest(jsonDoc, manifest);
        
        // Parse scene manifest
        parseSceneManifest(jsonDoc, manifest);
...
│       │   ├── AssetPackLoader.h/.cpp    # Loads assets from JSON or binary packs
...
│       ├── config/
│       │   ├── SceneConfigParser.h/.cpp  # Parses JSON scene definitions
...
│       ├── platform/
│       │   ├── IJsonParser.h              # Abstract interface for JSON parsing
│       │   └── RapidJsonParser.h/.cpp      # Concrete implementation using rapidjson
...
*   **Purpose:** Processes raw asset definitions (JSON) into optimized runtime formats for production. In debug, it can convert to an in-memory Intermediate Representation (IR). It handles schema validation, parameter extraction, and various optimizations.
*   **Key Methods:**
    *   `bool ProcessAssetConfiguration(const std::string& configFile)`: Parses a package JSON.
    *   `bool ValidateAgainstSchema(const std::string& json, const std::string& schemaFile)`: Validates JSON against JSON Schema.
...
*   **Interactions:**
    *   Uses `AssetSchema` for JSON Schema validation (conceptual class representing schema operations).
...
*   **Purpose:** Transforms high-level scene definitions (JSON) into optimized runtime representations. It validates scene configurations, resolves asset references (using `AssetId`s from `AssetCompiler`), and optimizes scene graphs.
*   **Key Methods:**
    *   `bool CompileScene(const std::string& sceneFile)`: Main function to process a scene JSON.
...
*   **Purpose:** Generates C++ headers or binary lookup tables that map human-readable asset names/paths (from JSON) to their **stable numeric `AssetId`s** used at runtime. This enforces the "strings only at the edges" principle.
*   **Interactions:** Reads asset and scene JSONs (or the IR from `AssetCompiler`) to gather all symbolic names requiring an `AssetId`. Outputs a `AssetIds.h` header file or a binary table.
...
*   **Key Methods:**
    ...
    *   `void registerSceneConfig(AssetId id, const std::string& sceneJson)`: Stores raw JSON for scene configs (in debug) or a binary scene blob (in release).
...
*   **Purpose:** The primary loader responsible for reading asset packages. In debug, it parses JSON files. In release, it memory-maps and parses binary `.pak` files.
*   **Key Methods:**
    *   `bool loadPackage(const std::string& packagePath)`: Reads a package file, parses its contents, and registers assets.
    *   `bool parseAssets(const std::string& jsonContent, const std::string& packageName)`: Extracts and registers asset definitions from JSON.
    *   `std::unique_ptr<TextureAsset> parseTextureAsset(const std::string& jsonNode)`: Parses specific asset types.
*   **Interactions:**
    *   Uses **`IJsonParser`** (`RapidJsonParser`) for JSON parsing in debug.
...
**3.2.3. `IJsonParser` and `RapidJsonParser` (`platform/IJsonParser.h/.cpp`, `RapidJsonParser.h/.cpp`)**
*   **Purpose:** `IJsonParser` defines a platform-independent interface for JSON parsing, adhering to the IoC principle. `RapidJsonParser` is a concrete implementation using the rapidjson library, injected into components needing JSON parsing.
*   **Key Methods (`IJsonParser`):** `virtual bool loadFile(const std::string& path) = 0`.
...
*   **Key Methods:**
    ...
    *   `void watchPackage(const std::string &packagePath)`: Adds a package file (JSON) to its watch list.
...
*   **Purpose:** Parses JSON scene configuration files into structured C++ data (`SceneConfig::Scene`). It handles schema validation and populates data structures like `Entity`, `Material`, `Animation`, etc..
*   **Key Methods:**
    *   `ParseResult parseSceneFile(const std::string &filePath)`: Loads and parses a scene JSON.
    *   `ParseResult parseSceneString(const std::string &jsonContent)`: Parses JSON content directly.
...
1.  **Authoring (JSON):** Developers modify asset definitions in JSON files (`assets/packages/DeveloperPackage/*.json`, `assets/schemas/*.schema.json`, `assets/params/*.json`).
2.  **File Watching (`AssetHotReloadSystem`):**
    *   At startup, `BootstrapSystem` loads the `DeveloperPackage/package.json`.
