...
bool BehaviorSystem::attachBehavior(Core::Entity& entity, const std::string& behaviorName, 
...
void BehaviorSystem::removeBehaviors(Core::EntityId entityId) {
...
void BehaviorSystem::removeBehavior(Core::EntityId entityId, const std::string& behaviorName) {
    auto it = entityBehaviors.find(entityId);
    if (it != entityBehaviors.end()) {
...
std::vector<Components::EntityBehavior*> BehaviorSystem::getBehaviors(Core::EntityId entityId) const {
...
Components::EntityBehavior* BehaviorSystem::getBehavior(Core::EntityId entityId, 
...
bool BehaviorSystem::hasBehaviors(Core::EntityId entityId) const {
    auto it = entityBehaviors.find(entityId);
...
size_t BehaviorSystem::createSceneBehaviors(const Assets::Package& package, const std::string& sceneName,
    ...
    for (const auto& entityDef : scene->entities) {
        ...
        Core::Entity* entity = entityIt->second;
        if (!entity) {
            continue;
        }
        
        // Create behaviors for this entity
        for (const auto& behaviorDef : entityDef.behaviors) {
            if (attachBehavior(*entity, behaviorDef.className, behaviorDef.params)) {
                behaviorCount++;
            }
        }
    }
...
BehaviorSystem::BehaviorInstance* BehaviorSystem::findBehaviorInstance(Core::EntityId entityId, 
                                                                      const std::string& behaviorName) const {
    auto it = entityBehaviors.find(entityId);
    if (it != entityBehaviors.end()) {
        for (const auto& instance : it->second) {
