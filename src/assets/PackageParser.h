...
namespace Assets
{
    ...
    private:
        ...
        std::unique_ptr<AssetDefinition> parseAsset(const XmlNode &assetNode, PackageLoadResult &result);
        std::unique_ptr<MaterialAssetData> parseMaterialAsset(const XmlNode &node, PackageLoadResult &result);
        std::unique_ptr<MeshAssetData> parseMeshAsset(const XmlNode &node, PackageLoadResult &result);
        std::unique_ptr<TextureAssetData> parseTextureAsset(const XmlNode &node, PackageLoadResult &result);
        std::unique_ptr<ShaderAssetData> parseShaderAsset(const XmlNode &node, PackageLoadResult &result);
        std::unique_ptr<AudioAssetData> parseAudioAsset(const XmlNode &node, PackageLoadResult &result);

        // Scene parsing methods
        std::unique_ptr<SceneDefinition> parseScene(const XmlNode &sceneNode, PackageLoadResult &result);
        std::unique_ptr<EntityDefinition> parseEntity(const XmlNode &entityNode, PackageLoadResult &result);
        std::unique_ptr<Component> parseComponent(const XmlNode &componentNode, PackageLoadResult &result);
